...
<template>
  ...
  <div class="warmup" v-if="warmupSets.length > 0">
    <button class="warmup-toggle" @click="expanded = !expanded">
      <span class="warmup-header">
        ...
        <span class="warmup-summary">{{ warmupSets.length }} sets trước khi vào working weight</span>
      </span>
    ...
    <div v-if="expanded" class="warmup-list">
      <div v-for="s in warmupSets" :key="s.set_number" class="warmup-set">
        ...
        <div class="ws-info">
          <span class="ws-weight">{{ s.weight }} kg</span>
          <span class="ws-reps">× {{ s.reps }} reps</span>
        </div>
  ...
  <!-- Bar weight - no warmup needed -->
  <div class="warmup warmup-none" v-else-if="isBarWeight">
    <span class="warmup-badge">WARM-UP</span>
    <span class="warmup-note">Bar trống (20 kg) — không cần warm-up set</span>
  </div>

  <!-- Error -->
  <div class="warmup warmup-error" v-else-if="loadError">
    <span class="warmup-note">Không tải được warm-up (weight: {{ workingWeight }} kg)</span>
  </div>
</template>
