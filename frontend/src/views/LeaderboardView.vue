...
<template>
  <div class="lb-view">
    ...
    <p class="section-desc">Total: Squat + Bench + Deadlift (working weight)</p>
    <div v-if="loading" class="loading">Loading...</div>
    <div v-else-if="entries.length === 0" class="empty">No one on the leaderboard yet.</div>
    <div v-else class="lb-list">
      <div v-for="e in entries" :key="e.user_id" class="card lb-row" :class="{ me: e.user_id === auth.user?.id }">
        <div class="lb-rank">{{ getMedal(e.rank) }}</div>
        <div class="lb-avatar" v-if="e.avatar_url"><img :src="e.avatar_url" :alt="e.nickname" referrerpolicy="no-referrer"></div>
        <div class="lb-avatar placeholder" v-else>{{ e.nickname[0]?.toUpperCase() }}</div>
        <div class="lb-info">
          <div class="lb-name">{{ e.nickname }}</div>
          <div class="lb-detail">S {{ e.squat_weight }} · B {{ e.bench_weight }} · D {{ e.deadlift_weight }}</div>
...
.lb-row { display: flex; align-items: center; gap: 12px; margin-bottom: 0; padding: 12px 14px; transition: border-color 0.2s; border: 1px solid transparent; }
...
.lb-avatar.placeholder { display: flex; align-items: center; justify-content: center; background: var(--surface-2); color: var(--text-dim); font-weight: 700; font-size: 16px; }
