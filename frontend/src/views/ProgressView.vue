...
onMounted(async () => {
  ...
  if (exercises.value.length > 0) {
    selectedId.value = exercises.value[0].id
  }
})
...
// Also load on mount after exercises arrive
...
function renderChart() {
  ...
  const ex = exercises.value.find(e => e.id === selectedId.value)
  ...
  <div class="progress-view">
    ...
    <div class="card stats-card" v-if="exercises.length">
      <div class="stat">
        ...
        <div class="stat-value">
          {{ exercises.find(e => e.id === selectedId)?.current_weight ?? 0 }} kg
        </div>
      </div>
      <div class="stat">
        <div class="stat-label">Fails</div>
        <div class="stat-value">
          {{ exercises.find(e => e.id === selectedId)?.fail_count ?? 0 }}/3
        </div>
      </div>
      <div class="stat">
    ...
    <div class="chart-container">
      ...
      <div v-if="entries.length === 0" class="chart-empty">
        Chưa có dữ liệu. Hoàn thành ít nhất 1 workout.
      </div>
    </div>
  </div>
</template>
